...
def get_read_scalar_value(sdl_doc: DocumentNode) -> dict:
    """
    Map of single‑ID query return types.

    Given a parsed GraphQL SDL DocumentNode defining a Query type,
    ...
    for field in query_def.fields:
        ...
        if (
            ...
            if isinstance(ret_base, NamedTypeNode):
                ...
                result[obj_name] = field.name.value

    return result

def get_read_vector_value(sdl_doc: DocumentNode) -> dict:
    """
    Map of nonNull list of nonNull object queries:
    vrací slovník {Typ: názevQueryFieldu}

    Expected pattern: field:type = NON_NULL(LIST(NON_NULL(NamedType)))
    """
    result = {}
        ...
        if len(args) == 1 and isinstance(args[0].type, NonNullTypeNode):
            ...
            for pt in union_def.types or []:  # NamedTypeNode
                ...
                if isinstance(pt, NamedTypeNode) and "Error" not in pt.name.value:
                    ...
                    if obj_def:
                        result[pt.name.value] = field.name.value
    ...
    for field in mutation_def.fields:
        ...
        if len(args) == 1 and isinstance(args[0].type, NonNullTypeNode):
            ...
            if isinstance(entity_type, NamedTypeNode):
                result[entity_type.name.value] = field.name.value
...
def get_cruds(sdl_doc: DocumentNode) -> dict:
    ...
    """
    single = get_read_scalar_value(sdl_doc)
    vector = get_read_vector_value(sdl_doc)
...
def build_input_type_params(sdl_doc: DocumentNode,
    ...
    for field in input_def.fields:  # type: InputValueDefinitionNode
        ...
        params.append(f"${field.name.value}: {type_str}")
...
def createTests(schema):
    ...
    def create_particular_test(sdl_doc, typename, optype, ops):
        @pytest.mark.asyncio
        async def test_func(SchemaExecutor):
            if optype == "readp":
                return await test_page(sdl_doc, ops, SchemaExecutor)
            elif optype == "read":
                return await test_scalar(sdl_doc, ops, SchemaExecutor)
            elif optype == "insert":
                return await test_insert(sdl_doc, ops, SchemaExecutor)
            elif optype == "update":
                return await test_update(sdl_doc, ops, SchemaExecutor)
            elif optype == "delete":
    ...
    for typename, ops in cruds.items():
        for optype in ("readp", "read", "insert", "update", "delete"):
            if optype in ops:
                test = create_particular_test(sdl_doc, typename, optype, ops)
                test_name = f"test_{optype}_{typename}"
